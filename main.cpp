...
uintptr_t kernaddr_find(const char* name, size_t& size) {
...
uintptr_t de_referance(uintptr_t address, unsigned int offset) {
	if (address == 0)
		return 0;
...
void Clean1() {
	...
	ntoskrnlBase = kernaddr_find("ntoskrnl.exe", ntoskrnlSize);
	...
	uintptr_t mmUnloadedDrivers = de_referance(mmUnloadedDriversPtr, 3);
...
NTSTATUS BBScanSection(IN PCCHAR section, IN PCUCHAR pattern, IN UCHAR wildcard, IN ULONG_PTR len, OUT PVOID* ppFound, PVOID base = nullptr)
{
	...
	size_t ntoskrnlSize = 0;
	if (nullptr == base)
		base = (PVOID)kernaddr_find("ntoskrnl.exe", ntoskrnlSize);
	if (base == nullptr)
